...
const resolvePropertyType = (type) => {
...
const generateProps = (declaration, enums, types) => {
...
exports.Dart = {
    getHeader(options, version) {
        return new Promise((accept, reject) => {
            fs_1.default.readFile(path_1.default.join(__dirname, "../../../src/generators/dart/", "dartTemplate.dart"), (err, data) => {
                if (err) {
                    reject(err);
                    return;
                }
                accept(`// ignore_for_file: camel_case_types, non_constant_identifier_names, constant_identifier_names
import 'dart:convert';
...
const schemaVersion = "${version}";\r\r\r${data.toString("utf8")}`);
    ...
    generateClassEntity(options, declaration, enums, types, declarations) {
        return `
...
`;
    ...
    generateInterfaces(options, declaration, enums, types) {
        return `
    class ${declaration.name} extends Serial{
        ${declaration.name}({props}) : super(props:props);
        static String entityName = "${declaration.name}";
...
${props}}
    ...
    `;
        for (let a = 0; a < apis.length; a++) {
            ...
            const className = declaration.alias.substring(0, 1).toString().toUpperCase() +
            ...
            for (let m = 0; m < declaration.methods.length; m++) {
                ...
                if (methodType.indexOf("List<") != -1) {
                    ...
                    methodInnerType = methodInnerType.substring(0, methodInnerType.length - 1);
                }
                let returnStatement = `
...
class ${options.namespace}{
    ...
    generateFooter(options, enums, types, entities) {
        let footer = '';
